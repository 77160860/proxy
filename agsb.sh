...
export uuid=${uuid:-''}; export port_vm_ws=${vmpt:-''}; export port_tr=${trpt:-''}; export port_hy2=${hypt:-''}; export port_vlr=${vlrt:-''}; export port_tuic=${tupt:-''}; export cdnym=${cdnym:-''}; export argo=${argo:-''}; export ARGO_DOMAIN=${agn:-''}; export ARGO_AUTH=${agk:-''}; export ippz=${ippz:-''}; export name=${name:-''}; export oap=${oap:-''}
...
EOF
    ...
    if [ -n "$tup" ]; then
        if [ -n "$port_tuic" ]; then
            if [ "$port_tuic" -lt 1 ] || [ "$port_tuic" -gt 65535 ]; then
                echo "错误: TUIC端口必须在1-65535之间"
                exit 1
            fi
            echo "$port_tuic" > "$HOME/agsb/port_tuic"
        elif [ ! -e "$HOME/agsb/port_tuic" ]; then
            port_tuic=$(shuf -i 10000-65535 -n 1)
            echo "$port_tuic" > "$HOME/agsb/port_tuic"
        fi
...
EOF
            ...
            if pgrep -f 'agsb/sing-box' >/dev/null 2>&1; then
                ...
                if grep -q '"tag": "tuic-sb"' "$HOME/agsb/sb.json" 2>/dev/null; then
                    p="$(cat "$HOME/agsb/port_tuic" 2>/dev/null)"
                    if port_in_use "$p"; then
                        echo "端口已被占用:$p (tuic),跳过 sing-box nohup 启动"
                        skip_sb="yes"
                    fi
                fi
...
cip(){
    ...
    if grep -q "tuic-sb" "$HOME/agsb/sb.json"; then port_tuic=$(cat "$HOME/agsb/port_tuic"); tuic_link="tuic://${uuid}:${uuid}@${server_ip}:${port_tuic}?congestion_control=bbr&udp_relay_mode=native&alpn=h3&sni=www.bing.com&allow_insecure=1#${sxname}tuic-$hostname"; echo "【 TUIC 】(直连协议)"; echo "$tuic_link" | tee -a "$HOME/agsb/jh.txt"; echo; fi
    ...
    if [ -n "$argodomain" ]; then
        ...
        if [ "$vlvm" = "Vmess" ]; then
            vmatls_link1="vmess://$(echo "{\"v\":\"2\",\"ps\":\"${sxname}vmess-ws-tls-argo-$hostname-443\",\"add\":\"saas.sin.fan\",\"port\":\"443\",\"id\":\"$uuid\",\"aid\":\"0\",\"net\":\"ws\",\"host\":\"$argodomain\",\"path\":\"/${uuid}-vm\",\"tls\":\"tls\",\"sni\":\"$argodomain\"}" | base64 -w0)"
            tratls_link1=""
...
if [ "$1" = "rep" ]; then cleandel; rm -rf "$HOME/agsb"/{sb.json,sbargoym.log,sbargotoken.log,argo.log,argoport.log,cdnym,name,port_tuic}; echo "重置完成..."; sleep 2; fi
