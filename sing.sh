...
EOF
    ...
    if [ -n "$stp" ]; then
        ...
        cat >> "$HOME/sing/sb.json" <<EOF
{"type": "shadowtls", "tag": "shadowtls", "listen": "::", "listen_port": ${port_st}, "version": 3, "users": [ { "password": "${st_password}" } ], "handshake": { "server": "www.bing.com", "server_port": 443 }, "strict_mode": true, "detour": "ss-in" },
...
cip(){
    ...
    if grep -q '"tag": "hy2"' "$HOME/sing/sb.json"; then port_hy2=$(cat "$HOME/sing/port_hy2"); hy2_link="hysteria2://$uuid@$server_ip:$port_hy2?security=tls&alpn=h3&insecure=1&sni=www.bing.com#${sxname}hy2-$hostname"; echo "【 Hysteria2 】"; echo "$hy2_link" | tee -a "$HOME/sing/jh.txt"; echo; fi
    if grep -q '"tag": "tuic"' "$HOME/sing/sb.json"; then port_tuic=$(cat "$HOME/sing/port_tuic"); tuic_link="tuic://${uuid}:${uuid}@${server_ip}:${port_tuic}?congestion_control=bbr&udp_relay_mode=native&alpn=h3&sni=www.bing.com&allow_insecure=1#${sxname}tuic-$hostname"; echo "【 TUIC 】"; echo "$tuic_link" | tee -a "$HOME/sing/jh.txt"; echo; fi
...
- {"name":"SS2022-shadowtls","type":"ss","server":"${server_ip}","port":${port_st},"cipher":"2022-blake3-aes-128-gcm","password":"${ss_password}","plugin":"shadow-tls","client-fingerprint":"chrome","plugin-opts":{"host":"www.bing.com","password":"${st_password}","version":3},"h2mux":{"enabled":true}}
EOF
        echo
    fi
