...
set_sbyx() {
    ...
    v4v6
    if (curl -s4m5 -k "$v46url" >/dev/null 2>&1) || (wget -4 -qO- --tries=2 "$v46url" >/dev/null 2>&1); then
        v4_ok=true
    fi
    if (curl -s6m5 -k "$v46url" >/dev/null 2>&1) || (wget -6 -qO- --tries=2 "$v46url" >/dev/null 2>&1); then
        v6_ok=true
    fi
...
EOF
    ...
    if [ -n "$vlr" ]; then
        ...
        if ! echo "$short_id" | grep -Eq '^[0-9a-fA-F]{16}$'; then
...
cip() {
    ...
    echo
    echo "聚合节点: cat $HOME/agsb/jh.txt"
    echo "========================================================="
    echo "相关快捷方式如下："
    showmode
}
