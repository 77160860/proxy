...
if [ "$1" = "list" ] || [ "$1" = "del" ] || [ "$1" = "res" ] || [ "$1" = "ups" ]; then
    ...
    if [ "$1" = "rep" ]; then
        [ "$vrp" = yes ] || [ "$trp" = yes ] || [ "$hyp" = yes ] || [ "$snp" = yes ] || [ "$ssp" = yes ] || { echo "提示:rep重置协议时,请在脚本前至少设置一个协议变量哦,再见!"; exit; }
    fi
else
    [ "$vrp" = yes ] || [ "$trp" = yes ] || [ "$hyp" = yes ] || [ "$snp" = yes ] || [ "$ssp" = yes ] || { echo "提示:未安装sing脚本,请在脚本前至少设置一个协议变量哦,再见!"; exit; }
fi
export uuid=${uuid:-''}; export port_tr=${tr:-''}; export port_hy2=${hy:-''}; export port_vrp=${vr:-''}; export port_snell=${sn:-''}; export port_ss=${ss:-''}; export cdnym=${cdnym:-''}; export argo=${argo:-''}; export ARGO_DOMAIN=${agn:-''}; export ARGO_AUTH=${agk:-''}; export ippz=${ippz:-''}; export name=${name:-''}; export oap=${oap:-''}
...
EOF
    ...
    if [ -n "$vrp" ]; then
        ...
        if [ -n "$port_vrp" ]; then
            if port_in_use "$port_vrp"; then old_port="$port_vrp"; port_vrp=$(get_free_port); echo "警告: VLESS-Reality 指定端口 $old_port 已被占用, 自动更换为新端口: $port_vrp"; fi
            echo "$port_vrp" > "$HOME/sing/port_vrp"
        elif [ -e "$HOME/sing/port_vrp" ]; then
            port_vrp=$(cat "$HOME/sing/port_vrp")
            if port_in_use "$port_vrp" || [[ ! "$port_vrp" =~ ^[0-9]+$ ]]; then old_port="$port_vrp"; port_vrp=$(get_free_port); echo "警告: VLESS-Reality 缓存端口 $old_port 异常或已被占用, 自动更换为新端口: $port_vrp"; echo "$port_vrp" > "$HOME/sing/port_vrp"; fi
        else
            port_vrp=$(get_free_port); echo "$port_vrp" > "$HOME/sing/port_vrp"
        fi
        port_vrp=$(cat "$HOME/sing/port_vrp"); echo "VLESS-Reality-Vision端口:$port_vrp"
...
EOF
    ...
    if [ -n "$ssp" ]; then
        ...
        if [ -n "$port_ss" ]; then
            if port_in_use "$port_ss"; then old_port="$port_ss"; port_ss=$(get_free_port); echo "警告: ShadowTLS 指定端口 $old_port 已被占用, 自动更换为新端口: $port_ss"; fi
            echo "$port_ss" > "$HOME/sing/port_ss"
        elif [ -e "$HOME/sing/port_ss" ]; then
            port_ss=$(cat "$HOME/sing/port_ss")
            if port_in_use "$port_ss" || [[ ! "$port_ss" =~ ^[0-9]+$ ]]; then old_port="$port_ss"; port_ss=$(get_free_port); echo "警告: ShadowTLS 缓存端口 $old_port 异常或已被占用, 自动更换为新端口: $port_ss"; echo "$port_ss" > "$HOME/sing/port_ss"; fi
        else
            port_ss=$(get_free_port); echo "$port_ss" > "$HOME/sing/port_ss"
        fi
        port_ss=$(cat "$HOME/sing/port_ss"); echo "SS2022端口:$port_ss"
        st_password="${uuid}"
        ss_password=$(openssl rand -base64 16)
        echo "$st_password" > "$HOME/sing/st_password"
        echo "$ss_password" > "$HOME/sing/ss_password"
        cat >> "$HOME/sing/sb.json" <<EOF
{"type": "shadowtls", "tag": "shadowtls", "listen": "::", "listen_port": ${port_ss}, "tcp_fast_open": true, "version": 3, "users": [ { "password": "${st_password}" } ], "handshake": { "server": "www.icloud.com", "server_port": 443 }, "strict_mode": true, "detour": "ss-in" },
...
- {"name":"SS2022-shadowtls","type":"ss","server":"${server_ip}","port":${port_ss},"tfo":true,"cipher":"2022-blake3-aes-128-gcm","password":"${ss_password}","plugin":"shadow-tls","client-fingerprint":"chrome","plugin-opts":{"host":"www.icloud.com","password":"${st_password}","version":3},"smux":{"enabled":true}}
EOF
        echo
    fi
...
if [ "$1" = "rep" ]; then cleandel; rm -rf "$HOME/sing"/{sb.json,sbargoym.log,sbargotoken.log,argo.log,argoport.log,cdnym,name,port_tr,port_vrp,port_snell,port_ss,reality.key,short_id,uuid,vlvm,server_ip.log,snell_psk,st_password,ss_password}; echo "重置完成..."; sleep 2; fi
