...
hostname=$(uname -n 2>/dev/null || cat /proc/sys/kernel/hostname 2>/dev/null || echo "host")
    ...
    case "$short_id" in
        (""|*[!0-9a-fA-F]*)
            if command -v openssl >/dev/null 2>&1; then
                ...
                short_id=$(dd if=/dev/urandom bs=4 count=1 2>/dev/null | od -An -tx1 | tr -d ' \n')
            fi
...
sb_version(){
    "$HOME/agsb/sing-box" version 2>/dev/null | sed -n 's/.*version[[:space:]]\{1,\}\([^[:space:]]\{1,\}\).*/\1/p'
}

cf_version(){
    "$HOME/agsb/cloudflared" version 2>/dev/null | sed -n 's/.* \([0-9][0-9.]*\).*/\1/p'
}
...
reality_public_key(){
    sed -n '2{s/.*[Pp]ublic[Kk]ey[:[:space:]]\{1,\}\([^[:space:]]\{1,\}\).*/\1/p;}' "$HOME/agsb/reality.key" | head -n1
}
...
ins(){
    ...
    if [ -n "$argo" ] && [ -n "$vmag" ]; then
        ...
        if [ "$argo" = "vmpt" ]; then argoport=$(cat "$HOME/agsb/port_vm_ws" 2>/dev/null); echo "Vmess" > "$HOME/agsb/vlvm";
        elif [ "$argo" = "trpt" ]; then argoport=$(cat "$HOME/agsb/port_tr" 2>/dev/null); echo "Trojan" > "$HOME/agsb/vlvm";
        fi
...
EOF
    ...
    if find /proc/*/exe -type l 2>/dev/null | grep -E '/proc/[0-9]+/exe' | xargs -r readlink 2>/dev/null | grep -Eq 'agsb/(sing-box|c)' \
        || pgrep -f 'agsb/sing-box' >/dev/null 2>&1 \
        || pgrep -f 'agsb/cloudflared' >/dev/null 2>&1 ; then

        [ -f "$HOME/.bashrc" ] || touch "$HOME/.bashrc"
        sed -i '/agsb/d' "$HOME/.bashrc" 2>/dev/null
        ...
        if ! pidof systemd >/dev/null 2>&1 && ! command -v rc-service >/dev/null 2>&1; then
            echo "if ! pgrep -f 'agsb/sing-box' >/dev/null 2>&1; then export cdnym=\"${cdnym}\" name=\"${name}\" ippz=\"${ippz}\" argo=\"${argo}\" uuid=\"${uuid}\" $vmp=\"${port_vm_ws}\" $trp=\"${port_tr}\" $hyp=\"${port_hy2}\" $vlr=\"${port_vlr}\" agn=\"${ARGO_DOMAIN}\" agk=\"${ARGO_AUTH}\"; bash \"$HOME/bin/agsb\"; fi" >> "$HOME/.bashrc"
        fi
        ...
        if ! pidof systemd >/dev/null 2>&1 && ! command -v rc-service >/dev/null 2>&1; then
            sed -i '/agsb\/sing-box/d' /tmp/crontab.tmp 2>/dev/null
            echo '@reboot sleep 10 && nohup $HOME/agsb/sing-box run -c $HOME/agsb/sb.json >/dev/null 2>&1 &' >> /tmp/crontab.tmp
        fi
        sed -i '/agsb\/cloudflared/d' /tmp/crontab.tmp 2>/dev/null
        if [ -n "$argo" ] && [ -n "$vmag" ]; then
            if [ -n "${ARGO_DOMAIN}" ] && [ -n "${ARGO_AUTH}" ]; then
                if ! pidof systemd >/dev/null 2>&1 && ! command -v rc-service >/dev/null 2>&1; then
...
agsbstatus(){
    ...
    if echo "$procs" | grep -Eq 'agsb/sing-box' || pgrep -f 'agsb/sing-box' >/dev/null 2>&1; then echo "Sing-box (版本V$(sb_version))：运行中"; else echo "Sing-box：未启用"; fi
    if echo "$procs" | grep -Eq 'agsb/c' || pgrep -f 'agsb/c' >/dev/null 2>&1; then echo "Argo (版本V$(cf_version))：运行中"; else echo "Argo：未启用"; fi
}

cip(){
    ipbest(){
        serip=$( (curl -s4m5 -k "$v46url") || (wget -4 -qO- --tries=2 "$v46url") )
        if echo "$serip" | grep -q ':'; then server_ip="[$serip]"; else server_ip="$serip"; fi
    ...
    if grep -q "hy2-sb" "$HOME/agsb/sb.json"; then
        ...
        echo "💣【 Hysteria2 】(直连协议)"; echo "$hy2_link" | tee -a "$HOME/agsb/jh.txt"; echo
    fi
    ...
    argodomain=$(cat "$HOME/agsb/sbargoym.log" 2>/dev/null)
    [ -z "$argodomain" ] && argodomain=$(argo_try_domain)
...
cleandel(){
    for P in /proc/[0-9]*; do
        if [ -L "$P/exe" ]; then
            ...
            if echo "$TARGET" | grep -qE '/agsb/c|/agsb/sing-box'; then kill "$(basename "$P")" 2>/dev/null; fi
        fi
    done
    ...
    crontab -l > /tmp/crontab.tmp 2>/dev/null
    sed -i '/agsb/d' /tmp/crontab.tmp 2>/dev/null
