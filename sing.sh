...
EOF
    ...
    if [ -n "$vlr" ]; then
        ...
        cat >> "$HOME/sing/sb.json" <<EOF
{"type": "vless", "tag": "vless-reality", "listen": "::", "listen_port": ${port_vlr}, "tcp_fast_open": true, "users": [{"uuid": "${uuid}","flow": "xtls-rprx-vision"}],"tls": {"enabled": true,"server_name": "www.ua.edu","reality": {"enabled": true,"handshake": {"server": "www.ua.edu","server_port": 443},"private_key": "${private_key}","short_id": ["${short_id}"]}}},
EOF
    fi
    if [ -n "$stp" ]; then
...
cip(){
    ...
    if grep -q '"tag": "hy2"' "$HOME/sing/sb.json"; then port_hy2=$(cat "$HOME/sing/port_hy2"); hy2_link="hysteria2://$uuid@$server_ip:$port_hy2?security=tls&sni=www.icloud.com#${sxname}hy2-$hostname"; echo "【 Hysteria2 】"; echo "$hy2_link" | tee -a "$HOME/sing/jh.txt"; echo; fi
    if grep -q '"tag": "tuic"' "$HOME/sing/sb.json"; then port_tuic=$(cat "$HOME/sing/port_tuic"); tuic_link="tuic://${uuid}:${uuid}@${server_ip}:${port_tuic}?congestion_control=bbr&udp_relay_mode=native&sni=www.icloud.com#${sxname}tuic-$hostname"; echo "【 TUIC 】"; echo "$tuic_link" | tee -a "$HOME/sing/jh.txt"; echo; fi
    if grep -q '"tag": "vless-reality"' "$HOME/sing/sb.json"; then
        ...
        vless_link="vless://${uuid}@${server_ip}:${port_vlr}?encryption=none&security=reality&sni=www.ua.edu&fp=chrome&flow=xtls-rprx-vision&publicKey=${public_key}&shortId=${short_id}&tfo=true#${sxname}vless-reality-$hostname"
