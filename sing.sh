...
EOF
    ...
    if [ -n "$tup" ]; then
        ...
        cat >> "$HOME/sing/sb.json" <<EOF
{"type": "tuic", "tag": "tuic", "listen": "::", "listen_port": ${port_tuic},"users": [ { "uuid": "${uuid}", "password": "${uuid}" } ],"tls": { "enabled": true, "certificate_path": "$HOME/sing/cert.pem", "key_path": "$HOME/sing/private.key" },"congestion_control": "bbr"},
EOF
    fi
}
sbbout(){
