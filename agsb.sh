...
agsbxurl="https://raw.githubusercontent.com/77160860/proxy/main/agsbx.sh" # Note: This is the source URL, kept for reference
showmode(){
    echo "Agsb脚本 (Sing-box内核版)"
    echo "主脚本：bash <(curl -Ls ${agsbxurl}) 或 bash <(wget -qO- ${agsbxurl})"
    echo "显示节点信息命令：agsb list"
    echo "重置变量组命令：自定义协议变量组 agsb rep"
    echo "更新Sing-box内核命令：agsb ups"
    echo "重启脚本命令：agsb res"
    echo "卸载脚本命令：agsb del"
    echo "---------------------------------------------------------"
}
echo "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"; echo "Agsb一键无交互脚本💣 (Sing-box内核版)"; echo "当前版本：V25.12.13"; echo "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
...
EOF
    ...
    if [ -n "$vlr" ]; then
        ...
        private_key=$(awk -F': *' 'tolower(\$1) ~ /private/ {print \$2; exit}' "$HOME/agsb/reality.key")

        if [ ! -f "$HOME/agsb/short_id" ] || [ -z "$(cat "$HOME/agsb/short_id" 2>/dev/null)" ]; then
        ...
        cat >> "$HOME/agsb/sb.json" <<EOF
{"type": "vless", "tag": "vless-reality-vision-sb", "listen": "::", "listen_port": ${port_vlr},"sniff": true,"users": [{"uuid": "${uuid}","flow": "xtls-rprx-vision"}],"tls": {"enabled": true,"server_name": "${reality_server}","reality": {"enabled": true,"handshake": {"server": "${reality_server}","server_port": ${reality_port}},"private_key": "${private_key}","short_id": ["${short_id}"]}}},
EOF
    fi
}
sbbout(){
...
EOF
    ...
    if find /proc/*/exe -type l 2>/dev/null | grep -E '/proc/[0-9]+/exe' | xargs -r readlink 2>/dev/null | grep -Eq 'agsb/(sing-box|c)' || pgrep -f 'agsb/(sing-box|c)' >/dev/null 2>&1 ; then
        [ -f ~/.bashrc ] || touch ~/.bashrc; sed -i '/agsb/d' ~/.bashrc; SCRIPT_PATH="$HOME/bin/agsb"; mkdir -p "$HOME/bin"; (curl -sL "$agsbxurl" -o "$SCRIPT_PATH") || (wget -qO "$SCRIPT_PATH" "$agsbxurl"); chmod +x "$SCRIPT_PATH"
...
cip(){
    ...
    if grep -q "hy2-sb" "$HOME/agsb/sb.json"; then port_hy2=$(cat "$HOME/agsb/port_hy2"); hy2_link="hysteria2://$uuid@$server_ip:$port_hy2?security=tls&alpn=h3&insecure=1&sni=www.bing.com#${sxname}hy2-$hostname"; echo "💣【 Hysteria2 】(直接连接)"; echo "$hy2_link" | tee -a "$HOME/agsb/jh.txt"; echo; fi
    if grep -q "vless-reality-vision-sb" "$HOME/agsb/sb.json"; then
        port_vlr=$(cat "$HOME/agsb/port_vlr")
        public_key=$(awk -F': *' 'tolower(\$1) ~ /public/ {print \$2; exit}' "$HOME/agsb/reality.key")
        short_id=$(cat "$HOME/agsb/short_id")
        ...
        echo "💣【 VLESS-Reality-Vision 】(直接连接)"; echo "$vless_link" | tee -a "$HOME/agsb/jh.txt"; echo;
    fi
